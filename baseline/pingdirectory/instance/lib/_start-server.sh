...
# 00763322 - remove cursor entry limit restrictions preventing purges from working
PRIVATE_UNBOUNDID_JAVA_ARGS="${PRIVATE_UNBOUNDID_JAVA_ARGS:+${PRIVATE_UNBOUNDID_JAVA_ARGS} }-Dcom.unboundid.directory.server.backends.jeb.AttributeIndex.cursorEntryLimit=0"
PRIVATE_UNBOUNDID_JAVA_ARGS="${PRIVATE_UNBOUNDID_JAVA_ARGS:+${PRIVATE_UNBOUNDID_JAVA_ARGS} }-Dcom.unboundid.directory.server.plugins.PurgeExpiredDataPlugin.searchCursorLimit=0"
